...
object B {
  ...
  def howManySymbols(n: Int, ans: Long): Long = {
    val len = String.valueOf(n).length
    if (len == 1) {
      return ans + n
    }
    return howManySymbols(Math.pow(10, len - 1).toInt - 1, ans + (n.toLong - Math.pow(10, len - 1) + 1).toLong * (len).toLong)
  }
  ...
  def solve: Int = {
    val n = nextInt
    out.println(howManySymbols(n, 0))
